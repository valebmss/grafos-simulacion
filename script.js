...
function getRandomColor() {
  ...
  function GenerateData() {
    ...
    for (let i = 1; i <= 50; i++) {
      let from = `Node ${Math.floor(Math.random() * 50) + 1}`;
      let to = `Node ${Math.floor(Math.random() * 50) + 1}`;
      let value = `Connection ${i}`;
      AddRelation(from, to, value);
    }
  }
...
function AddRelation(from = null, to = null, value = null) {
    if (!from || !to || !value) {
      // Si falta algún parámetro, muestra un mensaje de error
      alert("Faltan datos para agregar la relación.");
      return;
    }
